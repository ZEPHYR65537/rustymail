...
后续 M4 是持久化外发队列与固定上游中继。0.7.0 已新增 [M4.1 教程](17-m4-durable-queue.md)，实现队列存储、逐人结果、租约、退避与未知结果；真正中继及退信循环保护仍待 M4.2/M4.3。现有“任何外域 RCPT 都拒绝”的矩阵在 M4.2 须改成按认证、路由和完整接受责任授权；不能先放行外域再补接受责任。
