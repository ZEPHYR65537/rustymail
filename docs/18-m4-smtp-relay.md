...
**Linux 强杀后的管理 socket：**正常退出会删除自己创建的 socket；SIGKILL 不执行析构，路径可能残留。当前服务拒绝覆盖任何已有路径，不会自动判断并删除旧 socket。恢复时先停止自动拉起，确认旧进程已退出、实例锁可取得，再核对配置中私有目录里的确切 socket 路径、所有者及文件类型，清理该遗留 socket 后重启；不要删除数据库锁或不明文件。独立强杀实验在等待自己启动的子进程退出后，核对 socket 的设备号/inode、类型和私有目录权限，再执行这一步。该实验验证的是受控恢复，不是无人值守自动重启。生产自动重启和运行目录管理留待 M7 验收。
