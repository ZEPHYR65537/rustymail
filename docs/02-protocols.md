...
本页包含生产目标设计；实际 SMTP 输入语法、回复、上限和未实现形式沿用 [M3.3 契约](16-m3-input-contract.md)。0.7.0 新增 [M4.1 持久队列 API](17-m4-durable-queue.md)，但 SMTP 仍仅环回、本地交付；表中的远程收件人、通知和 IMAP 尚未启用。
...
远端最终 250 后本地更新状态失败，或 DATA 发完后 TCP 断开，均存在结果不确定窗口。保留原文和投递状态；M4.1 存储 API 在 body 边界后结果不明时进入 uncertain，要求管理员显式允许重复后再重试。实际网络映射和长期处理策略待 M4.2/M4.3。不能承诺重复很少，也不能用 Message-ID 自动删除业务上合法的重复消息。未来运维指标须单列 uncertain 数量。
