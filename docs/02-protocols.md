...
本页包含生产目标设计；实际 SMTP 输入语法、回复、上限和未实现形式沿用 [M3.3 契约](16-m3-input-contract.md)。0.8.0 已启用[固定上游 TLS 中继与原子混合提交](18-m4-smtp-relay.md)；入站仍仅环回，远端 RCPT 只允许显式中继模式的认证提交。通知、IMAP 与直接 MX 尚未实现。
...
| 能力 | 首发策略 | 附加要求 |
| --- | --- | --- |
| SIZE | 支持，最大客户端内容 25 MiB | 当前按上限加 2 KiB 预约，实际输入独立计数；生成头部不侵占客户端上限 |
...
目标是在提交之前检查配额、反垃圾和授权。0.5.0 已实现本地交付的 Received/Return-Path，见[教程](15-m3-local-delivery.md)：Received 使用实际对端 IP、语法受限的客户端名称和本机信息，不代表客户端名称已通过 DNS 验证。原始外部 Authentication-Results 不被当作本机证明。M6 入站 DKIM 必须在任何头部变换前验证；本步不重写正文，域认证和反垃圾仍待实现。
...
远端最终 250 后本地更新状态失败，或 DATA 发完后 TCP 断开，均存在结果不确定窗口。保留原文和投递状态；M4.1 存储 API 在 body 边界后结果不明时进入 uncertain，要求管理员显式允许重复后再重试。M4.2 已验证真实网络映射；长期处理与通知策略待 M4.3。不能承诺重复很少，也不能用 Message-ID 自动删除业务上合法的重复消息。未来运维指标须单列 uncertain 数量。

初始重试计划是项目策略：30 分钟、1 小时、2 小时、4 小时，之后每 4 小时；加入 0–20% 正向抖动，默认寿命 5 天。M4.1 已实现逐任务延期、小批索引扫描和每域活动额度；到期先 hold。目的域共享退避仍是后续目标，不能让一个失败域使 10 万条邮件同时再次连接。队列持久化 `next_attempt_at_ms`，禁止全部装入堆内存。
...
对 8-bit 内容：如果下一跳不支持 8BITMIME，首发不实现自动 MIME 转码；明确失败并生成通知。上线中继应预先验收该扩展。不能为了兼容把已签名邮件静默重编码。这是完整目标；M4.2 对不兼容能力先 hold，尚无通知。当前出站表示为共享 blob 去掉已验证的首行 Return-Path；需要签名或复杂变换时再引入独立且明确版本的变体。
