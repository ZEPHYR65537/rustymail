...
接受事件在尝试发送最终 250 之前提交给诊断日志。0.3.1 改为有界后台输出，满队列、慢输出或强制退出可能丢事件，因此不能用“没有日志”证明没有提交。数据库中的 operation 记录才是持久查询依据。日志不记录信封地址、正文或认证负载，SMTP 的最终响应仍以数据库完成提交为前提。队列策略见[资源边界教程](13-cancellation-and-bounds.md)。
