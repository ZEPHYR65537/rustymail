...
两平台测试、Linux 故障回归、条款映射和原始数据见 [M3.2 验证报告](../reports/m3.2/validation.md)。

当前三个 SMTP 入口及旧实验命令均使用本章流程，只交付本地邮箱。无外发、MIME 语义解析、域认证或反垃圾，仍只允许环回实验。0.6.0 的输入契约与压力实验见 [M3.3](16-m3-input-contract.md)。
...
为了可靠识别字段边界，当前策略拒绝无字段名的首个续行、字段名带空格、无冒号和头部控制字符等输入；结构错误返回 550 并关闭，避免把未读 DATA 当成命令。NUL、裸 CR/LF 和超长物理行由分帧层拒绝：0.5.0 使用 451，0.6.0 改为 554；DATA 超时改为 421。0.6.0 还要求 ASCII 头部，8-bit 正文必须显式声明 BODY=8BITMIME。认证提交仍额外执行 M2 的受限 From 检查。
...
继续阅读 [M3.3](16-m3-input-contract.md)：统一输入契约、能力矩阵、语法/状态负例和资源压力验收。所有实验通过也不会解除公网生产门槛。
