...
完整设计仍待逐项验收。L0 已落地 ADR 002/003 的基础路径，并增加进程/VM 故障验证；M2 落地 ADR 006/011 的身份与计算预算。不等于完成生产耐久性与全部功能。实际行为见[首轮教程](10-first-implementation.md)、[M1 教程](11-m1-storage.md)和 [M2 教程](12-m2-identity.md)，当前依赖见 [M2 清单与审计](../reports/m2/validation.md)。重要变更需记录原因，不让代码、教程和运维文档各说各话。
...
| 015 | 单实例队列按收件人持久化；guard 跟随实际工作和正文读者；body 后未知结果暂停，过期先 hold；每批索引扫描最多 128 行 | 仅靠租约时间重派会与旧网络工作重叠；自动重试 uncertain 提高进展但可能重复；保守暂停需要运维介入 | M4.2 必须验证真实网络生命周期；M4.3 定义通知、过期与重复策略，规模增长后复评扫描公平性；见 [M4.1 教程](17-m4-durable-queue.md) |
| 016 | 先用单收件人真实 SMTP 尝试验证状态，再接 TLS 和队列；认证混合提交共享 blob，出站严格省略已生成首行 Return-Path；本地与远端责任原子提交 | 逐收件人握手降低批量吞吐；读取投影节省正文副本，但必须验证前缀和整个文件 EOF | 连接池/PIPELINING 接入前验证部分结果；M6 签名前重新定义版本化变体；见 [M4.2 教程](18-m4-smtp-relay.md) |
