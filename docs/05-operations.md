...
> 状态：这是生产服务的目标操作契约。当前 L0 已有环回 SMTP、M1 存储维护，以及 M2 的 TLS、应用密码和 Unix 管理接口；实际命令以 [M1 教程](11-m1-storage.md)、[M2 教程](12-m2-identity.md)和 `--help` 为准。下文生产服务、完整备份、队列等能力仍待实现，不能直接照做部署。DNS / OpenSSL 检查可以在已有环境使用，但 example.com、192.0.2.10、密钥和账号均为占位符。
