...
后续发现的 Unix 管理 socket 强杀恢复步骤见 [M4.2 恢复手册](18-m4-smtp-relay.md#7-可复现实验与配置)：残留路径需要在确认旧进程退出后核对并清理，服务不会自动覆盖。

本章基于 0.3.0，并补充 0.3.1 的资源与取消修复。代码继续运行在 L0 环回实验环境，提供隐式 TLS、AUTH PLAIN、应用密码、send-as、撤销、Unix 管理 socket 和证书重载。原始阶段验收见 [M2 报告](../reports/m2/validation.md)。M3 补齐本地 SMTP，IMAP 属于 M5，公网生产还需 M6/M7；本阶段不开放外域投递或生产启动入口。
